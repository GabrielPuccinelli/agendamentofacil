...
export default function ManageServices({ memberId, organizationId }: Props) {
  ...
  const [price, setPrice] = useState(0); // Corrigido: Adicionado estado para o preço
  ...
  const createServiceMutation = useMutation(
    ...
    async ({ name, duration, price }: { name: string; duration: number; price: number }) => {
      ...
      // Corrigido: Adicionado 'price' no insert
    ...
    },
    {
      onSuccess: () => {
        ...
        setName("");
        setDuration(30);
        setPrice(0); // Corrigido: Limpa o campo de preço
      },
    }
  );
  ...
  const handleCreateService = (e: React.FormEvent) => {
    e.preventDefault();
    createServiceMutation.mutate({ name, duration, price }); // Corrigido: Passa o preço
  };
    ...
    <div className="mt-6">
      ...
      {organizationId && (
        <form onSubmit={handleCreateService} className="p-4 border rounded-md bg-gray-50 mb-6">
          ...
          {/* CORRIGIDO: Adicionadas labels e melhorado o layout */}
