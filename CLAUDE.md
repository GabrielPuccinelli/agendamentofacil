...
- Serviços flexíveis: `services.unit_label` (ex.: sessão/aula) + `services.is_product` (item à venda) + `duration` default 0. ManageServices tem 3 tipos (Por horário / Por sessão-unidade / Produto), categoria livre com datalist. PublicPage só lista agendáveis (duration>0, !is_product).
