...
- Aba **Produtos** (`/company/sales`): vendas separadas dos atendimentos (filtra por `services.is_product`); resumo (total/unidades/média), histórico clicável → diálogo editável (quantidade/forma/excluir, ajusta estoque), mais vendidos. A aba **Agendamentos** exclui vendas de produto.
