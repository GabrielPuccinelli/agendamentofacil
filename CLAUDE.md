...
- Serviços flexíveis: `services.unit_label` (ex.: sessão/aula) + `services.is_product` (item à venda) + `services.stock` (estoque, null=não controla) + `duration` default 0. ManageServices tem 3 tipos (Por horário / Por sessão-unidade / Produto), categoria livre com datalist. PublicPage só lista agendáveis (duration>0, !is_product).
- **Venda de produto** (`SellProductDialog`, botão no DashboardPage): registra venda avulsa como booking status `completed`/paid, `bookings.quantity` + `bookings.amount` (total), e baixa estoque via RPC `decrement_stock`. **Receita usa `amount ?? services.price`** em todos os cálculos (CompanyDashboard + DayOverview).
